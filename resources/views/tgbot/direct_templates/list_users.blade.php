    ...
    <div class="row main_bot_view">
        <div class="col-xs-5 col-md-2">
            ...
            <div class="btn-group-vertical" role="group" aria-label="...">
                ...
                <a class="btn btn-default active" href="/bot_admin/list_users" role="button">Пользователи</a>
        ...
        <div class="col-xs-6">
            <h2 class="current_menu">Пользователи</h2>
            <div class="view_result" id="view_result">
                <table class="table table-striped">
                    ...
                    @foreach($data as $el)
                        <tr>
                            <td>{{ $el->id }}</td>
                            <td>{{  ($el->username_tg and $el->username_tg != 0) ? '@' . $el->username_tg : (($el->id_telegram and $el->id_telegram !=0)? $el->id_telegram : '')}}</td>
                            <td>
                                @if ($el->subscribes->where('active',1)->first())
                                    @foreach($el->subscribes->where('active',1) as $sub)
                                        <p><b>{{$sub->tariff ? $sub->tariff->name : 'Неизвестный тариф'}} ({{$sub->subscribe_type}} до {{$sub->finish_date_show}})</b></p>
                                    @endforeach
                                @else
                                    нет подписок
                                @endif
                            </td>
                            <td>Зарегестрирован {{ $el->user_register}} </td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
